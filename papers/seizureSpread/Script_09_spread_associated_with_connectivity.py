...
for i in range(len(patientsWithseizures)):
    ...
    if len(connectivity_loc_glob) > 0:
        ...
        sc_temporal_mean = np.nanmean(sc_temporal)
        sc_LR_mean = np.nanmean(sc_temporal_L_R)
        #get quickness
        m = 0
...
sc_vs_quickness["inverse_quickness"] = 1/sc_vs_quickness["quickenss"]

sc_vs_quickness= sc_vs_quickness.fillna(0)

sc_vs_quickness_group = sc_vs_quickness.groupby(["subject"], as_index=False).mean()


g = sns.regplot(data = sc_vs_quickness, x = "sc_LR_mean", y= "quickenss")
x = sc_vs_quickness["sc_LR_mean"]
y = sc_vs_quickness["quickenss"]
y_nanremoved = y[~np.isnan(y)]
x_nanremoved = x[~np.isnan(y)]
spearmanr(x_nanremoved,y_nanremoved)


g = sns.regplot(data = sc_vs_quickness_group, x = "sc_LR_mean", y= "quickenss")
x = sc_vs_quickness_group["sc_LR_mean"]
y = sc_vs_quickness_group["quickenss"]
y_nanremoved = y[~np.isnan(y)]
x_nanremoved = x[~np.isnan(y)]
spearmanr(x_nanremoved,y_nanremoved)
...
x_nanremoved = x[~np.isnan(y)]
spearmanr(x_nanremoved,y_nanremoved)
pearsonr(x_nanremoved,y_nanremoved)
