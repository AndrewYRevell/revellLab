...
try:
    ...
    rerun = sys.argv[10]
    print(f"Rerun argument given: {rerun}")
    if rerun == "True":
        rerun = True
    if rerun == "False":
        rerun = False
    if not type(rerun) is bool:
        print ("argument must be given as True or False")
...
if rerun:
    ...
    for i in range(len(clean)):
        if utils.checkIfFileExists(clean[i]):
            utils.executeCommand(f"rm -r {clean[i]}")
